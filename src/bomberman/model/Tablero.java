...
public abstract class Tablero extends Observable {
	...
	public void moverBomberman(int pX, int pY) {
		Object[] array = Arrays.stream(casillas).flatMap(fila -> Arrays.stream(fila))
				.filter(c -> c.tieneBomberman() && c.getEstadoBomberman().equals("vivo")).toArray();

		Casilla c = (Casilla) array[0];
		int nuevoX = c.getX() + pX;
		int nuevoY = c.getY() + pY;
		if (!(nuevoX < 0 || nuevoX >= getRows() || nuevoY < 0 || nuevoY >= getCols()
				|| casillas[nuevoX][nuevoY].tieneBloque())) {
	...
	public void moverEnemigo(int x, int y, int id) {
		...
		if (x - 1 >= 0 && !casillas[x - 1][y].tieneBloque() && !casillas[x - 1][y].tieneEnemigo()
				&& !casillas[x - 1][y].tieneExplosion() && !(x - 2 >= 0 && casillas[x - 2][y].tieneEnemigo())
				&& !(y - 1 >= 0 && casillas[x - 1][y - 1].tieneEnemigo())
				&& !(y + 1 < getCols() && casillas[x - 1][y + 1].tieneEnemigo())) {// arriba
			movimientos.add("Arriba");
		}
		if (x + 1 < getRows() && !casillas[x + 1][y].tieneBloque() && !casillas[x + 1][y].tieneEnemigo()
				&& !casillas[x + 1][y].tieneExplosion() && !(x + 2 < getRows() && casillas[x + 2][y].tieneEnemigo())
				&& !(y - 1 >= 0 && casillas[x + 1][y - 1].tieneEnemigo())
				&& !(y + 1 < getCols() && casillas[x + 1][y + 1].tieneEnemigo())) {// abajo
			movimientos.add("Abajo");
		}
		if (y - 1 >= 0 && !casillas[x][y - 1].tieneBloque() && !casillas[x][y - 1].tieneEnemigo()
				&& !casillas[x][y - 1].tieneExplosion() && !(y - 2 >= 0 && casillas[x][y - 2].tieneEnemigo())
				&& !(x - 1 >= 0 && casillas[x - 1][y - 1].tieneEnemigo())
				&& !(x + 1 < getRows() && casillas[x + 1][y - 1].tieneEnemigo())) {// izq
			movimientos.add("Izquierda");
		}
		if (y + 1 < getCols() && !casillas[x][y + 1].tieneBloque() && !casillas[x][y + 1].tieneEnemigo()
				&& !casillas[x][y + 1].tieneExplosion() && !(y + 2 < getCols() && casillas[x][y + 2].tieneEnemigo())
				&& !(x - 1 >= 0 && casillas[x - 1][y + 1].tieneEnemigo())
				&& !(x + 1 < getRows() && casillas[x + 1][y + 1].tieneEnemigo())) {// der
